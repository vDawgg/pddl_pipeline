...
class DSPyToolCallPipelineFull(DSPyToolCallPipeline):
    ...
    def generate_plan(self, domain_file: str, problem_file: str) -> str:
        ...
        plan_output = self._generate_plan(
            Path(domain_file), Path(problem_file), UnsolvabilityFeedback.FULL
        )
    ...
    def __init__(
        ...
        ToolCallPipeline.__init__(
            self, model, domain, pipeline or Pipelines.DSPY_TOOL_CALL_FULL
        )
