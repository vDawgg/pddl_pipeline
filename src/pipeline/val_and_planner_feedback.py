...
class ValAndPlannerFeedbackPipeline(ValFeedbackPipeline):
    ...
    def fix_plan_not_found(
        ...
        if image_paths is not None:
            ...
            unformatted_prompt = get_prompt(
                Prompts.PLANNER_CONTEXT_IMAGES, Prompts.PLANNER_TASK
            )
    ...
    def fix_parsing_error(
        ...
        if planner_output.file == PDDLFiles.DOMAIN:
            content = make_file_view(self._read_pddl_file(domain_file))
        else:
            content = make_file_view(self._read_pddl_file(problem_file))
        prompt = unformatted_prompt.format(
            file=planner_output.file,
            err_msg=planner_output.error_message,
            content=content,
        )
        output, _ = self.make_request(prompt, img_paths=image_paths)
        if planner_output.file == PDDLFiles.DOMAIN:
    ...
    def fix_planning(
        ...
        for i in range(num_tries):
            ...
            if type(planner_output) is str:
                ...
                iterations = i + 1
                self.fix_parsing_error(
                    domain_file, problem_file, planner_output, image_paths
                )
            else:
                logger.debug("Planning error")
                iterations = i + 1
                self.fix_plan_not_found(domain_file, problem_file, image_paths)
