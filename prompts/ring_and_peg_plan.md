...
Your task is to try to generate a plan from previously generated PDDL domain and problem files, which are noted below. If the current implementation of domain/problem does not result in a plan, fix the existing files until a plan can be generated. As the source of truth for whether a plan can be generated you will use the fast downward planning system. In addition to the file paths to the domain and problem file, you will also receive a domain and problem description in natural language to make sure that your changes still align with this description.
